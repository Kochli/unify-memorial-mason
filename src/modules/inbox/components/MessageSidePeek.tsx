    ...
    <div className="w-[450px] bg-white border-l h-screen fixed right-0 top-0 shadow-2xl z-[60] flex flex-col animate-in slide-in-from-right duration-300">
      <div className="p-6 bg-slate-900 text-white flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-blue-500 rounded-xl flex items-center justify-center font-black">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-white/40 uppercase tracking-widest flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-green-500" /> Active Channel
            </p>
          </div>
        </div>
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col ${msg.type === 'inbound' ? 'items-start' : 'items-end'}`}>
            <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-xs font-medium leading-relaxed shadow-sm border ${
              msg.type === 'inbound' ? 'bg-white border-slate-100 text-slate-800 rounded-tl-none' : 
              msg.type === 'ai' ? 'bg-blue-600 border-blue-500 text-white rounded-tr-none' :
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase mt-1 px-1">{msg.timestamp}</span>
          </div>
        ))}
      </div>
      ...
      <div className="p-6 border-t bg-white">
        <div className="flex gap-2 p-1.5 bg-slate-50 rounded-2xl border border-slate-200">
          ...
          />
          <button className="bg-slate-900 text-white p-3 rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10">
