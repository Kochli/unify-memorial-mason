    ...
    <div className="p-10 max-w-[1600px] mx-auto space-y-10">
      <div className="flex justify-between items-end">
        ...
        <div className="flex items-center gap-8">
           <div className="flex flex-col items-end">
             <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Contract Value</p>
             <div className="flex items-center gap-2">
                <span className="text-3xl font-black text-slate-900 tracking-tighter">£{totalValue.toLocaleString()}</span>
             </div>
           </div>
           <div className="w-px h-12 bg-slate-200" />
           <div className="flex flex-col items-end">
             <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1 text-green-600">Total Collected</p>
             <div className="flex items-center gap-2">
                <span className="text-3xl font-black text-green-600 tracking-tighter">£{totalPaid.toLocaleString()}</span>
                <div className="p-1.5 bg-green-50 rounded-lg text-green-600"><ArrowUpRight className="w-4 h-4" /></div>
             </div>
           </div>
           <div className="w-px h-12 bg-slate-200" />
           <div className="flex flex-col items-end">
             <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1 text-orange-500">Outstanding Balance</p>
             <span className="text-3xl font-black text-orange-500 tracking-tighter">£{totalBalance.toLocaleString()}</span>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
         <div className="bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between group">
            ...
            <div className="relative z-10">
              ...
              <h4 className="text-5xl font-black tracking-tighter">£{(totalPaid * 0.9).toLocaleString(undefined, { minimumFractionDigits: 2 })}</h4>
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <div className="px-10 py-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
           <h3 className="text-xl font-black text-slate-900 tracking-tight">Financial Stream</h3>
                ...
                <tr key={order.id} className="hover:bg-slate-50/80 transition-all group cursor-pointer" onClick={() => onViewOrder(order.id)}>
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-5">
                      <div className="w-12 h-12 bg-slate-100 rounded-[1rem] flex items-center justify-center font-black text-slate-600">{order.customerName.charAt(0)}</div>
                      <div>
                        <p className="font-black text-slate-900">{order.customerName}</p>
                        <p className="text-[10px] text-slate-400 font-bold mt-0.5">{order.deceasedName}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-10 py-6">
                  ...
                  <td className="px-10 py-6 text-right">
                    <span className="text-lg font-black text-slate-900">£{paid.toLocaleString()}</span>
...
const ActionCard = ({ icon, title, desc, color, onClick }: any) => {
  ...
  return (
    <button onClick={onClick} className={`flex flex-col items-start p-10 rounded-[2.5rem] border-2 transition-all group text-left h-full ${colorMap[color]}`}>
      <div className="p-4 rounded-2xl bg-white shadow-xl mb-6 group-hover:scale-110 transition-transform duration-500">{React.cloneElement(icon as any, { className: 'w-8 h-8' })}</div>
